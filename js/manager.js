  ...
  Manager.prototype = {
    ...
    */
    ...
    readCourses: function (evt) {
      var reader, name, self;
      reader = new FileReader();
      reader.onerror = function () {
        rg2.utils.showWarningDialog('Course file error', 'The selected course file could not be read.');
      };
      self = this;
      name = evt.target.files[0].name.toLowerCase();
      this.courseFileType = name.substr(name.length - 3);
      reader.onload = function (evt) {
        self.processCourseFile(evt);
      };
      if (this.courseFileType === "pdf") {
    ...
    processCourseFile: function (evt) {
      ...
      this.newcontrols.deleteAllControls();
      if (this.courseFileType === "pdf") {
        parsedCourses = new rg2.OOMCourseParser(this.localworldfile);
        pdfjsLib.getDocument(evt.target.result).promise
          .then(pdf => pdf.getPage(1))
          .then(page => page.getTextContent())
          .then(textContent => parsedCourses.processCoursePDF(textContent))
          .then(courseData => this.saveParsedCourses(courseData))
          .catch(err => rg2.utils.showWarningDialog("OOM PDF file error", "File is not a valid OOM PDF file. " + err));
      } else {
        parsedCourses = new rg2.CourseParser(evt, this.worldfile, this.localworldfile);
        this.saveParsedCourses(parsedCourses);
      }
    },

    saveParsedCourses: function (parsedCourses) {
